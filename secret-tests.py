    ...
    def run(self, *functions):
        ...
        for function in functions:
            ...
            if not self.failed:
                ...
                self.print("Remember:")
                self.print("""
                    Please check server.py for:
                        * unapproved imports
                        * defeating the test code
                        * any other funny business
                    Actually, do this before you run this script next time if you haven't already.
                """)
...
deeper_index = cleandoc("""
    <!DOCTYPE html>
    <html>
    <head>
        <title>Deeperer Example Page</title>
            <meta http-equiv="Content-Type"
            content="text/html;charset=utf-8"/>
            <!-- check conformance at http://validator.w3.org/check -->
            <link rel="stylesheet" type="text/css" href="deeper.css">
    </head>
...
def one_giant_function(tester):
    ...
    with tester("Making www dir..."):
        ...
        with tester("writing files"):
            ...
            deeper_path = deep_path / "deeper"
            if not deeper_path.is_dir():
                deeper_path.mkdir()
            
            deeper_index_path = deeper_path / "index.html"
            deeper_index_path.write_text(deeper_index)

            deeper_css_path = deeper_path / "deeper.css"
            deeper_css_path.write_text(deeper_css)
    ...
    with tester("/deep/deeper"):
        with tester("GET /deep/deeper"):
            response = get("deep/deeper")
            assert response.status in [301, 308], f"Expected code 301 got {response.status}"
            assert 'Location' in response.headers, f"Didn't find location header"
            location = response.headers['Location']
            assert location.endswith('/'), location
        ...
        with tester("following redirect"):
            response = get(response.url, location)
            assert response.status == 200, f"Expected code 200 got {response.status}"
            same_text(deeper_index, response.read())
            check_mime("text/html", response)
            
            with tester("deep/deeper/deeper.css"):
                response = get(response.url, "deeper.css")
                same_text(deeper_css, response.read())
                check_mime("text/css", response)
